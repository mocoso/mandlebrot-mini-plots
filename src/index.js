...
function scaleForIterations(iterations) {
  const max = Math.max(
    ...iterations.map((c) => Math.max(Math.abs(c.re), Math.abs(c.im)))
  );
...
function complexPlotToCoordinates(n, m, c, scale) {
  ...
  const middle = {
    x: topLeftOfPlot.x + plotSize / 2,
    y: topLeftOfPlot.y + plotSize / 2,
  };
...
function plotToComplex(n, m) {
  ...
  return middle.add(
    new Complex(
      (coords.x - ctx.width / 2) * pixelsToComplexScale,
      -((coords.y - ctx.height / 2) * pixelsToComplexScale)
    )
  );
}

function saveFile(ctx) {
  const blob = new Blob([ctx.getSerializedSvg()], {
